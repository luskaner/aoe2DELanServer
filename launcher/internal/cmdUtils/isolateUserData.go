...
func (c *Config) IsolateUserData(game string, metadata bool, profiles bool) (errorCode int) {
	if metadata || profiles {
		...
		if result := executor.RunSetUp(game, nil, nil, nil, metadata, profiles, false, false); !result.Success() {
